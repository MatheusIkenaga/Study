...
char entrasn(){
	...
	if(sn != 'S'&&sn != 'N')
		do{
			printf("Resposta inv�lida, insira novamente (S/N) \n");
			scanf("%c",&sn);
			fflush(stdin);	
		}while((sn!='S'&&sn !='N'));
	return sn;
...
int main(void) {
	...
	do {
		printf("***PRESSIONE A TECLA ENTER PARA COME�AR***\n");
		scanf("%c", &resp1);
		fflush(stdin);
		system("cls");
		...
		printf("2 - Sorteio\n");
		printf("3 - Informa��es sobre a cadeira \n");
		printf("4 - Sair\n\n");
		scanf("%c", &resp1);
		fflush(stdin);
		system("cls");
		...
		case '1':
			...
			if((contdef==25) && (contconv==20) && (contagem ==100)){
				...
				case 'S':
				if(contconv > 20){
						printf("N�O H� LUGARES DISPON�VEIS PARA CONVIDADOS");
					}else{
						printf("\nDigite seu nome\n");
				...
				case 'N':	
					...
					case 'S':
						if(contdef > 25){
							printf("N�O H� MAIS LUGARES DISPON�VEIS PARA DEFICIENTES \n ");
						}else{
							printf("\nDigite seu nome\n");
					...
					case 'N':
						if(contagem > 100){
							printf("N�O H� LUGARES DISPON�VEIS PARA N�O CONVIDADOS OU N�O DEFICIENTES");
						}else{
		...
		case '2':
			...
			break;
		case '3':
			printf("DIGITE O N�MERO DA CADEIRA\n\n");
			cadeira = entrai();
